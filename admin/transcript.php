...
$students_res = $conn->query("
    SELECT s.student_id, s.roll_number, u.full_name as name, s.department, s.program
    FROM students s
...
if ($selected_student_id > 0) {
    ...
    if ($student) {
        ...
        $stmt2->bind_param("i", $selected_student_id);
        $stmt2->execute();
...
<div class="page-header no-print">
    <h1>Search Student Transcripts</h1>
</div>

<!-- Student Selector -->
<div style="background: white; padding: 20px; border-radius: var(--radius); box-shadow: var(--shadow); margin-bottom: 25px;" class="no-print">
    <form method="GET" style="display: flex; gap: 15px; align-items: flex-end;">
        <div class="form-group" style="margin-bottom: 0; flex: 1; min-width: 300px;">
            <label style="font-weight: 600; color: var(--dark);">Select Student to Generate Transcript</label>
...
<?php if ($selected_student_id > 0 && $student): ?>
    <?php if (empty($semesters_data)): ?>
        <div class="alert alert-warning no-print">No published results found for this student. Transcripts can only display results from published exam sessions. Please publish this student's exam results in the Result Center first.</div>
    <?php else: ?>
        <div style="text-align: right; margin-bottom: 20px;" class="no-print">
            <button onclick="window.print()" class="btn" style="background:var(--primary);"><span style="margin-right:8px;">🖨️</span> Print Official Transcript</button>
        </div>
...
<?php elseif ($selected_student_id > 0): ?>
    <div class="alert alert-error no-print">Selected student profile not found.</div>
<?php endif; ?>
